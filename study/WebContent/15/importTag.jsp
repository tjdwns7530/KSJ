...
<body>
	...
	<c:out value="out 태그가 <br> 출력한 내용." escapeXml="false"></c:out>
		
		<form action="importTag.jsp" method="get">
			<select name="where">
				<option value="nexearch" ${param.where eq 'nexearch' ? 'selected="selected"' : '' }>통합</option>
				<option value="article" ${param.where eq 'article' ? 'selected="selected"' : '' }>카페</option>
				<option value="post" ${param.where eq 'post' ? 'selected="selected"' : '' }>블로그</option>
